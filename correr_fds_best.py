...
archivo = "archivos terminal//terminal fds best LISTA G -- " + file_name.replace("grafos//grafo_", "")[:-7] + ".txt"
...
for k in valores_k:
    ...
    # nombre = file_name + f"--fds_best--k={k} - LMCUT.pickle"
    nombre = file_name + f"--fds_best--k={k} LMCUT NUEVO.pickle"
    file = open(nombre, "wb")
    pickle.dump(dato, file)
    nombre_g = file_name + f"--fds_best--k={k} LISTA G.pickle"
